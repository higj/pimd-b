...
FactorialBosonicExchange::FactorialBosonicExchange(const BosonicExchangeContext& context)
: BosonicExchangeBase(context), m_labels(context.nbosons) {
...
double FactorialBosonicExchange::getMinExteriorSpringEnergy() {
    ...
    do {
        ...
        min_delta = std::min(min_delta, 0.5 * m_context.spring_constant * diff2);
...
double FactorialBosonicExchange::effectivePotential() {
    ...
    return (-1.0 / m_context.thermo_beta) * log(weights_sum / permutation_counter);
}
    ...
    do {
        ...
        for (int l = 0; l < m_context.nbosons; ++l) {
            ...
            for (int axis = 0; axis < NDIM; ++axis) {
                ...
                temp_force(l, axis) = diff_next[axis] * m_context.spring_constant;
            }
        }

        weight = exp(-m_context.thermo_beta * (0.5 * m_context.spring_constant * weight - m_e_shift));
    ...
    do {
        ...
        for (int l = 0; l < m_context.nbosons; ++l) {
            ...
            for (int axis = 0; axis < NDIM; ++axis) {
                ...
                //temp_force(l, axis) = sums[axis] * m_context.spring_constant;
                temp_force(l, axis) = -diff_prev[axis] * m_context.spring_constant;
            }
        }
...
double FactorialBosonicExchange::primitiveEnergyEstimator() {
    ...
    do {
        ...
        const double delta_spring_energy = 0.5 * m_context.spring_constant * weight;
        weight = exp(-m_context.thermo_beta * (delta_spring_energy - m_e_shift));
...
#if IPI_CONVENTION
    bosonic_spring_energy /= m_context.nbeads;
#endif
