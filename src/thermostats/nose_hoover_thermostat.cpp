    ...
    m_mass(mass)
{
    ...
    // the timescale on which the heat-bath variables evolve.
    ...
    Qi = Constants::hbar * Constants::hbar * thermal_ctx.beta / m_nbeads;

    // Q1/Qi should be equal to the number of degrees of freedom (NDOF).
    // In the absence of constraints, NDOF=NDIM*natoms.
    Q1 = NDIM * m_natoms * Qi;

    eta = std::vector(nchains, 0.0);
    eta_dot = std::vector(nchains, 0.0);
    eta_dot_dot = std::vector(nchains, 0.0);

    dt2 = 0.5 * dt;
    dt4 = 0.25 * dt;
    dt8 = 0.125 * dt;

    required_energy = NDIM * m_natoms / thermal_ctx.thermo_beta;
}

double NoseHooverThermostat::getAdditionToH()
{
    return singleChainGetAdditionToH(NDIM * m_natoms, 0);
}

double NoseHooverThermostat::singleChainGetAdditionToH(const int& ndof, const int& index) const
{
    ...
    addition_to_h += ndof * eta[index] / m_thermal_ctx.thermo_beta;

    // Similar calculation, but for the rest of the thermostats
    for (int i = 1; i < m_nchains; i++)
    {
        addition_to_h += 0.5 * Qi * eta_dot[index + i] * eta_dot[index + i];
        addition_to_h += eta[index + i] / m_thermal_ctx.thermo_beta;
    }
...
double NoseHooverThermostat::singleChainStep(const double& current_energy, const int& index)
{
    ...
    eta_dot[m_nchains - 1 + index] += eta_dot_dot[m_nchains - 1 + index] * dt4;

    // Update the first derivative of eta for all others components
    for (int i = m_nchains - 2; i >= 0; i--)
    {
    ...
    double Q_former = Q1;
    for (int i = 1; i < m_nchains - 1; i++)
    {
        exp_factor = exp(-dt8 * eta_dot[i + 1 + index]);
        eta_dot[i + index] *= exp_factor;
        eta_dot_dot[i + index] = (Q_former * eta_dot[i - 1 + index] * eta_dot[i - 1 + index] - 1 / m_thermal_ctx.
            thermo_beta) / Qi;
        eta_dot[i + index] += eta_dot_dot[i + index] * dt4;
        eta_dot[i + index] *= exp_factor;
        Q_former = Qi;
    ...
    eta_dot_dot[m_nchains - 1 + index] = (Qi * eta_dot[m_nchains - 2 + index] * eta_dot[
        m_nchains - 2 + index] - 1 / m_thermal_ctx.thermo_beta) / Qi;
    eta_dot[m_nchains - 1 + index] += eta_dot_dot[m_nchains - 1 + index] * dt4;
...
NoseHooverNpThermostat::NoseHooverNpThermostat(
    const ThermalContext& thermal_ctx,
    const NormalModesContext& nm_ctx,
    const std::shared_ptr<SystemState>& state,
    int nchains,
    double dt,
    double mass
) : NoseHooverThermostat(thermal_ctx, nm_ctx, state, nchains, dt, mass)
{
    Q1 = NDIM * Qi;
    const int eta_length = nchains * m_natoms;
    eta = std::vector(eta_length, 0.0);
    eta_dot = std::vector(eta_length, 0.0);
    eta_dot_dot = std::vector(eta_length, 0.0);

    required_energy = NDIM / thermal_ctx.thermo_beta;
}
...
void NoseHooverNpThermostat::momentaUpdate()
{
    for (int ptcl_idx = 0; ptcl_idx < m_natoms; ++ptcl_idx)
    {
        ...
        // Obtain the scaling factor
        double scale = singleChainStep(current_energy, ptcl_idx * m_nchains);
...
double NoseHooverNpThermostat::getAdditionToH()
{
    ...
    for (int ptcl_idx = 0; ptcl_idx < m_natoms; ++ptcl_idx)
    {
        additionToH += singleChainGetAdditionToH(NDIM, ptcl_idx * m_nchains);
    }
    return additionToH;
}
...
NoseHooverNpDimThermostat::NoseHooverNpDimThermostat(
    const ThermalContext& thermal_ctx,
    const NormalModesContext& nm_ctx,
    const std::shared_ptr<SystemState>& state,
    int nchains,
...
) : NoseHooverThermostat(thermal_ctx, nm_ctx, state, nchains, dt, mass)
{
    ...
    const int eta_length = nchains * m_natoms * NDIM;
    eta = std::vector(eta_length, 0.0);
    eta_dot = std::vector(eta_length, 0.0);
    eta_dot_dot = std::vector(eta_length, 0.0);

    required_energy = 1 / thermal_ctx.thermo_beta;
}
...
void NoseHooverNpDimThermostat::momentaUpdate()
{
    for (int ptcl_idx = 0; ptcl_idx < m_natoms; ++ptcl_idx)
    {
        for (int axis = 0; axis < NDIM; ++axis)
        {
            double momentum_for_calc = m_coupling->getMomentumForCalc(ptcl_idx, axis);
            // Calculates the current energy in the system
            double current_energy = momentum_for_calc * momentum_for_calc / m_mass;

            // Obtain the scaling factor
            double scale = singleChainStep(current_energy, (ptcl_idx * NDIM + axis) * m_nchains);
...
double NoseHooverNpDimThermostat::getAdditionToH()
{
    ...
    for (int ptcl_idx = 0; ptcl_idx < m_natoms; ++ptcl_idx)
    {
        for (int axis = 0; axis < NDIM; ++axis)
        {
            addition_to_h += singleChainGetAdditionToH(1, (ptcl_idx * NDIM + axis) * m_nchains);
        }
    }
    return addition_to_h;
}
